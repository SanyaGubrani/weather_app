...
def current_weather(city):
    ...
    if response.status_code == 200:
        ...
        current_weather_result.config(text=f"Today's Weather in {city.upper()}:\n\nTemperature: {temp} °C\nHumidity: {humidity}%\nCondition: {desc}")
...
current_weather_btn = tk.Button(buttons_frame, text="Current Weather", font=("Arial", 14), command=lambda: current_weather(city_entry.get()))
current_weather_btn.pack(side='left', padx=5)

forecast_btn = tk.Button(buttons_frame, text="5-day Forecast", font=("Arial", 14), command=lambda: get_forecast(city_entry.get()))
